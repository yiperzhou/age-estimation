...
class Multi_Regression_MobileNet_V1_Model(torch.nn.Module):
    def __init__(self, args, age_classes=100):
        super(Multi_Regression_MobileNet_V1_Model, self).__init__()
    
        mobilenet_v1_model = Elastic_MobileNet()
        
        self.Multi_Regression_MobileNet_V1_model_feature = mobilenet_v1_model.model
        ...
        self.age_divide_100_classes, self.age_divide_20_classes, self.age_divide_10_classes, self.age_divide_5_classes = self.get_age_rgs_class()


        self.age_rgs_100_classes = nn.Sequential(

            
        )
        self.age_rgs_20_classes = nn.Sequential(

        )

        self.age_rgs_10_classes = nn.Sequential(

        )

        self.age_rgs_5_classes = nn.Sequential(
    ...
    def get_age_rgs_class(self):
        ...
        if self.args.age_rgs_combination == [1, 0, 0, 0]:
            age_divide_100_classes = True
            age_divide_20_classes = False
            age_divide_10_classes = False
            age_divide_5_classes = False       
                
        elif self.args.age_rgs_combination == [1, 1, 0, 0]:
            age_divide_100_classes = True
            age_divide_20_classes = True
            age_divide_10_classes = False
            age_divide_5_classes = False        
            
        elif self.args.age_rgs_combination == [1, 1, 1, 0]:
            age_divide_100_classes = True
            age_divide_20_classes = True
            age_divide_10_classes = True
            age_divide_5_classes = False                    

        elif self.args.age_rgs_combination == [1, 1, 1, 1]:
            age_divide_100_classes = True
            age_divide_20_classes = True
            age_divide_10_classes = True
    ...
    def get_age_regression_loss(self):
        return 0


    def forward(self, x):
        x = self.Multi_Regression_MobileNet_V1_Model(x)
