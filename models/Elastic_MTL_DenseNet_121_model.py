        ...
        for i, num_layers in enumerate(block_config):
            ...
            if i != len(block_config) - 1:
                ...
                if self.add_intermediate_layers == 2:
                    self.intermediate_CLF.append(IntermediateClassifier(num_features, self.num_categories[i]))
                    print("self num_categories: ", self.num_categories[i])
                    self.num_outputs += 1
...
def Elastic_MTL_DenseNet_121_model(args, logfile):

    num_classes = [2, 2, 7, 100] # tasks for Gender, Smile, Emotion, Age 
    add_intermediate_layers = 2
    pretrained_weight = 1

    model_weight_url = None

    if args.model == "Elastic_MTL_DenseNet_121_model":
    ...
    elif args.model == "Elastic_MTL_DenseNet_169_model":
    ...
    model.classifier = nn.Linear(in_features, num_classes[-1])  # num_classes[-1] = 100, 0-99 age
