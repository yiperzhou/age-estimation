...
def Train_Cvpr_age_gender(model, trainloader, criterion, optimizer, epoch, logFile, args):

    LOG("[train]: Starting, Epoch: " + str(epoch), logFile)
    ...
    for (i, (gender_img, gender_label)), (j, (age_img, age_label)), (k, (emo_img, emo_label)) in zip(enumerate(gender_train_loader), enumerate(age_train_loader), enumerate(emotion_train_loader)):
        ...
        if i < 10000000:
            ...
            # age
            gender_out_1, age_out_1 = model(age_img)
            ...
            # gender
            gender_out_2, age_out_2 = model(gender_img)
            ...
            # loss = age_loss * reduce_age_cls_loss + gender_loss * reduce_gen_loss + emo_loss * reduce_emo_loss
            loss = age_loss * 1 + gender_loss * 1
    ...
    LOG("[Train] losses: " + str(losses), logFile)
    LOG("[Train] age mae: " + str(age_epoch_mae.avg), logFile)

    LOG("[Train] emotion acc: " + str(emo_epoch_acc.avg), logFile)
    LOG("[Train] emotion loss: " + str(emo_epoch_loss.avg), logFile )
...
def Valid_Cvpr_age_gender(model, validloader, criterion, optimizer, epoch, logFile, args):
    ...
    for (i, (gender_img, gender_label)), (j, (age_img, age_label)), (k, (emo_img, emo_label)) in zip(enumerate(gender_valid_loader), enumerate(age_valid_loader), enumerate(emotion_valid_loader)):
        # print("i: ", i, ", inside the for loop")
