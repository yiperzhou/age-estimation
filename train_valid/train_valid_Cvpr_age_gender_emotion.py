...
def Train_Valid_Cvpr_age_gender_emotion(model, trainloader, criterion, optimizer, epoch, logFile, args, pharse):
    ...
    for (i, (gender_img, gender_label)), (j, (age_img, age_label)), (k, (emo_img, emo_label)), (m, (smile_img, smile_label)) in zip(enumerate(gender_train_loader), enumerate(age_train_loader), enumerate(emotion_train_loader), enumerate(smile_train_loader)):
        ...
        if i < 10000000:
            ...
            reduce_age_cls_loss, reduce_gen_loss, reduce_emo_loss  = args.weights[0], args.weights[1], args.weights[2]

            

            loss = age_loss * reduce_age_cls_loss + gender_loss * reduce_gen_loss + emo_loss * reduce_emo_loss + smile_loss * 1
            ...
            print("[", pharse," LOSS] total, Age, Gender, Emotion, Smile : ", loss.item(), age_loss.item(), gender_loss.item(), emo_loss.item(), smile_loss.item())
    ...
    LOG(" args.weights[0], args.weights[1], args.weights[2]: " + str(args.weights[0]) + ", " + str(args.weights[1]) + ", " + str(args.weights[2]), logFile)
...
def Valid_Cvpr_age_gender_emotion(model, validloader, criterion, optimizer, epoch, logFile, args):
    ...
    gender_criterion, age_criterion, smile_criterion, age_cls_criterion, emotion_criterion = criterion[0], criterion[1], criterion[2], criterion[3], criterion[4]
    gender_valid_loader, age_valid_loader, smile_valid_loader, emotion_valid_loader = validloader[0], validloader[1], validloader[2], validloader[3]


    for (i, (gender_img, gender_label)), (j, (age_img, age_label)), (k, (emo_img, emo_label)), (m, (smile_img, smile_label)) in zip(enumerate(gender_valid_loader), enumerate(age_valid_loader), enumerate(emotion_valid_loader), enumerate(smile_valid_loader)):
        # print("i: ", i, ", inside the for loop")
        ...
        if i < 10000000:
            ...
            # age
            gender_out_1, age_out_1, emo_out_1, smile_out_1 = model(age_img)
            ...
            # gender
            gender_out_2, age_out_2, emo_out_2, smile_out_2 = model(gender_img)
            # print("gender_out_2: ", gender_out_2)
            # gender_target = gender_label.view(-1)
            gender_loss = gender_criterion(gender_out_2, gender_label)
            gender_epoch_loss.update(gender_loss.item(), gender_label.size(0))
            ...
            gender_out_3, age_out_3, emo_out_3, smile_out_3 = model(emo_img)
            emo_loss = emotion_criterion(emo_out_3, emo_label)
            ...
            # smile
            gender_out_4, age_out_4, emo_out_4, smile_out_4 = model(smile_img)
            ...
            # reduce_age_mae      = 0.1
            # reduce_emo_loss     = 1
            reduce_age_mae, reduce_gen_loss, reduce_emo_loss  = args.weights[0], args.weights[1], args.weights[2]

            loss = age_loss * reduce_age_mae + gender_loss * reduce_gen_loss + emo_loss * reduce_emo_loss + smile_loss * 1

            print("[VALID LOSS] total, Age, Gender, Emotion, Smile : ", loss.item(), age_loss.item(), gender_loss.item(), emo_loss.item(), smile_loss.item())
    ...
    # # valid emotion task
    # for m, (emo_img, emo_label) in enumerate(emotion_valid_loader):
    #     # emotion
    #     emo_img = emo_img.cuda()
    #     emo_label = emo_label.cuda()
    ...
    LOG("[Valid] losses: " + str(losses), logFile)
    
    LOG("[Valid] age mae: " + str(age_epoch_mae.avg), logFile)

    LOG("[Valid] emotion acc: " + str(emo_epoch_acc.avg), logFile)
    LOG("[Valid] emotion loss: " + str(emo_epoch_loss.avg), logFile)
    
    LOG("[Valid] Smile acc: " + str(smile_epoch_acc.avg), logFile)
    LOG("[Valid] Smile loss: " + str(smile_epoch_loss.avg), logFile)
